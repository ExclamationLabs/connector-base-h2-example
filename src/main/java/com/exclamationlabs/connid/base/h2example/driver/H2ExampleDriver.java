...
/**
 * This is an example Base connector driver using a just-in-time H2 in-memory database for Identity
 * Access Management.
 */
public class H2ExampleDriver extends BaseDriver<H2ExampleConfiguration> {
  ...
  public void initialize(
      H2ExampleConfiguration baseConnectorConfiguration, Authenticator authenticator)
    ...
    try {
      ...
      sql =
          "CREATE TABLE DEMO_GROUPS (ID INTEGER NOT NULL, NAME VARCHAR(255), "
              + "DESCRIPTION VARCHAR(255), PRIMARY KEY (id))";
      stmt.executeUpdate(sql);

      sql =
          "CREATE TABLE DEMO_POWERS (ID INTEGER NOT NULL, NAME VARCHAR(255), "
              + "DESCRIPTION VARCHAR(255), PRIMARY KEY (id))";
      stmt.executeUpdate(sql);

      sql =
          "CREATE TABLE DEMO_USERS_XREF (user_id INTEGER NOT NULL, "
              + "group_id INTEGER NOT NULL, PRIMARY KEY (user_id, group_id))";
      stmt.executeUpdate(sql);

      sql =
          "CREATE TABLE DEMO_USERS_POWERS_XREF (user_id INTEGER NOT NULL, "
              + "power_id INTEGER NOT NULL, PRIMARY KEY (user_id, power_id))";
      stmt.executeUpdate(sql);
      stmt.close();

      H2ExampleUserInvocator userInvocator =
          (H2ExampleUserInvocator) getInvocator(H2ExampleUser.class);
      userInvocator.createInitialUsers(this);

      H2ExampleGroupInvocator groupInvocator =
          (H2ExampleGroupInvocator) getInvocator(H2ExampleGroup.class);
      groupInvocator.createInitialGroups(this);

      H2ExamplePowerInvocator powerInvocator =
          (H2ExamplePowerInvocator) getInvocator(H2ExamplePower.class);
